 ...
 * kiss_fft_scalar = float (single precision).
 *
...
struct kiss_fft_state {
    ...
    kiss_fft_cpx twiddles[1]; /* room for nfft twiddles (allocated past struct) */
};

/* float-scalar arithmetic macros (fixed-point scaling is a no-op) */
...
static void kf_bfly_generic(kiss_fft_cpx* Fout, const size_t fstride, const kiss_fft_cfg st, int m, int p) {
    ...
    kiss_fft_cpx t;
    int Norig = st->nfft;
    kiss_fft_cpx* scratch = (kiss_fft_cpx*)KISS_FFT_MALLOC(sizeof(kiss_fft_cpx) * (size_t)p);
    if (!scratch) return;
...
static void kf_work(kiss_fft_cpx* Fout, const kiss_fft_cpx* f, const size_t fstride,
                    int in_stride, const int* factors, const kiss_fft_cfg st) {
    ...
    if (m == 1) {
        ...
        do {
            /* recursive call: DFT of size m*p performed by doing p instances of smaller DFTs */
    ...
    switch (p) {
        case 2:
            ...
            break;
        case 3:
            kf_bfly3(Fout, fstride, st, (size_t)m);
            break;
        case 4:
            kf_bfly4(Fout, fstride, st, (size_t)m);
            break;
        case 5:
...
static void kf_factor(int n, int* facbuf) {
    ...
    do {
        while (n % p) {
            ...
            if ((double)p > floor_sqrt) p = n; /* no more factors, skip to end */
        }
        n /= p;
        *facbuf++ = p;
        *facbuf++ = n;
...
void kiss_fft(kiss_fft_cfg cfg, const kiss_fft_cpx* fin, kiss_fft_cpx* fout) {
    ...
    if (fin == fout) {
        kiss_fft_cpx* tmpbuf = (kiss_fft_cpx*)KISS_FFT_MALLOC(sizeof(kiss_fft_cpx) * (size_t)cfg->nfft);
        if (!tmpbuf) return;
        kf_work(tmpbuf, fin, 1, 1, cfg->factors, cfg);
        memcpy(fout, tmpbuf, sizeof(kiss_fft_cpx) * (size_t)cfg->nfft);
...
void kiss_fft_cleanup(void) {
    /* no static state in this implementation */
}

int kiss_fft_state_nfft(kiss_fft_cfg cfg) {
    return cfg ? cfg->nfft : 0;
}

int kiss_fft_state_inverse(kiss_fft_cfg cfg) {
    return cfg ? cfg->inverse : 0;
}
